...
// strides ([number, number, number]|number) The strides of the pooling: [strideDepth, strideHeight, strideWidth].
// If strides is a single number, then strideDepth == strideHeight == strideWidth.
...
// dataFormat ('NDHWC'|'NCDHW') An optional string from: "NDHWC", "NCDHW". Defaults to "NDHWC".
// Specify the data format of the input and output data. With the default format "NDHWC",
// the data is stored in the order of: [batch, depth, height, width, channels].
// Only "NDHWC" is currently supported. Optional
